...
import com.shcj.cache.entity.AppClientValueDistriSimple;
...
public class AppDailyDataCenterImpl implements AppDailyDataCenter {
    ...
    public int sendAppDailyEmail() {
        return collectAppDaily(true);
    }

    @Override
    public int collectAppDaily(boolean notify) {
        ...
        for (AppDesc appDesc : appDescList) {
            try {
                if (collectAppDaily(appDesc.getAppId(), startDate, endDate, notify)) {
                    successCount++;
                }
            } catch (Exception e) {
    ...
    public boolean sendAppDailyEmail(long appId, Date startDate, Date endDate) {
        return collectAppDaily(appId, startDate, endDate, true);
    }

    private boolean collectAppDaily(long appId, Date startDate, Date endDate, boolean notify) {
        try {
    ...
    private void fillAppDailyData(AppDailyData appDailyData) {
        ...
        if (MapUtils.isNotEmpty(valueSizeDistributeCountMap)) {
    ...
    public AppDailyData generateAppDaily(long appId, Date startDate, Date endDate) {
        return assembleAppDaily(appId, startDate, endDate, true);
    }

    @Override
    public AppDailyData buildAppDailySnapshot(long appId, Date startDate, Date endDate) {
        AppDailyData appDailyData = assembleAppDaily(appId, startDate, endDate, false);
        if (appDailyData != null) {
            fillAppDailyData(appDailyData);
        }
        return appDailyData;
    ...
    private AppDailyData assembleAppDaily(long appId, Date startDate, Date endDate, boolean forPersist) {
        ...
        if (forPersist && appDesc.isTestOk()) {
            return null;
        }
        ...
        Map appClientGatherStat = MapUtils.getMap(appClientGatherStatMap, appId);
    ...
    public void noticeAppDaily(Date startDate, AppDetailVO appDetailVO, AppDailyData appDailyData) {
        ...
        String title = String.format("【Redis管理平台】%s日报(appId=%s)", startDateFormat, appDetailVO.getAppDesc().getAppId());
        AppDesc appDesc = appDetailVO.getAppDesc();
    ...
    public void noteAppTopologyDaily(Date startDate, List<Map> topologyExamResult) {
        String startDateFormat = DateUtil.formatYYYYMMdd(startDate);
        String title = String.format("【Redis管理平台】应用拓扑结构检查结果%s日报", startDateFormat);
        Map<String, Object> context = new HashMap<>();
