...
public class RedisClusterAppDeployTask extends BaseTask {
    ...
    public TaskFlowStatusEnum init() {
        ...
        if (CollectionUtils.isEmpty(redisMasterMachineList)) {
            throw new BizException("task {} redisMasterMachineStr is empty", taskId);
        }
        ...
        if (CollectionUtils.isEmpty(redisSlaveMachineList)) {
            throw new BizException("task {} redisSlaveMachineList is empty", taskId);
        }
        ...
        if (StringUtils.isNotBlank(redisServerNodesStr)) {
            ...
            if (MapUtils.isEmpty(clusterMap)) {
                ...
                logger.info("init cluster map :{} successfully", clusterMap);
            }
        }
        // redis版本
    ...
    private void checkResourceAllowPerIp(String redisServerIp) {
        ...
        long memoryFreeMB = NumberUtils.toLong(machineStats.getMemoryFree()) / 1024 / 1024;
        long memoryNeedMB = maxMemory;
        if (memoryNeedMB > memoryFreeMB * 0.85) {
            throw new BizException("{} need {} MB, but memoryFreeMB is {} MB", redisServerIp, memoryNeedMB, memoryFreeMB);
        }
    }
    ...
    public TaskFlowStatusEnum checkMachineConnect() {
        // redis server
        for (String redisServerIp : redisMasterMachineList) {
            if (!checkMachineIsConnect(redisServerIp)) {
                throw new BizException("cluster: redisServer {} is not connected", redisServerIp);
            }
        }
        for (String redisServerIp : redisSlaveMachineList) {
            if (!checkMachineIsConnect(redisServerIp)) {
    ...
    public TaskFlowStatusEnum generateInstanceNodes() {
        ...
        redisServerNodes = instancePortService.generateRedisServerNodeList(appId, redisMasterMachineList, redisSlaveMachineList, maxMemory, AppTypeEnum.REDIS_CLUSTER);
        if (CollectionUtils.isEmpty(redisServerNodes)) {
            throw new BizException("redisServerNodes is empty, appId is {}, redisMasterMachineList is {}, redisSlaveMachineList is {}, maxMemory is {}",
    ...
    public TaskFlowStatusEnum startRedisCluster() {
        ...
        if (!isCluster) {
            throw new BizException("startRedisCluster--> create redis Cluster error :{}!", clusterMap);
        }
        return TaskFlowStatusEnum.SUCCESS;
    }
            ...
            for (Map.Entry<Jedis, Jedis> node : clusterMap.entrySet()) {
                ...
                try {
                    ...
                    if (isOk) {
                        ...
                        throw new BizException(" app :{} is not cluster , clusterInfo :{}", appId, clusterInfo);
                    }
                } catch (Exception e) {
    ...
    public TaskFlowStatusEnum updateMachineAllocateStatus(int status) {
        try {
            ...
            allMachineSet.addAll(redisMasterMachineList);
