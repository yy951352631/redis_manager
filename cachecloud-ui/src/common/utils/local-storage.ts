// 统一处理 localStorage

import type { ThemeName } from "@@/composables/useTheme"
import type { SidebarClosed, SidebarOpened } from "@@/constants/app-key"
import type { LayoutsConfig } from "@/layouts/config"
import type { TagView } from "@/pinia/stores/tags-view"
import { CacheKey } from "@@/constants/cache-key"

// #region Token
export function getToken() {
  return localStorage.getItem(CacheKey.TOKEN)
}

export function setToken(token: string) {
  localStorage.setItem(CacheKey.TOKEN, token)
}

export function removeToken() {
  localStorage.removeItem(CacheKey.TOKEN)
}
// #endregion

// #region 系统布局配置
export function getLayoutsConfig() {
  const json = localStorage.getItem(CacheKey.CONFIG_LAYOUT)
  return json ? (JSON.parse(json) as LayoutsConfig) : null
}

export function setLayoutsConfig(settings: LayoutsConfig) {
  localStorage.setItem(CacheKey.CONFIG_LAYOUT, JSON.stringify(settings))
}

export function removeLayoutsConfig() {
  localStorage.removeItem(CacheKey.CONFIG_LAYOUT)
}
// #endregion

// #region 侧边栏状态
export function getSidebarStatus() {
  return localStorage.getItem(CacheKey.SIDEBAR_STATUS)
}

export function setSidebarStatus(sidebarStatus: SidebarOpened | SidebarClosed) {
  localStorage.setItem(CacheKey.SIDEBAR_STATUS, sidebarStatus)
}
// #endregion

// #region 正在应用的主题名称
export function getActiveThemeName() {
  return localStorage.getItem(CacheKey.ACTIVE_THEME_NAME) as ThemeName | null
}

export function setActiveThemeName(themeName: ThemeName) {
  localStorage.setItem(CacheKey.ACTIVE_THEME_NAME, themeName)
}
// #endregion

// #region 标签栏
export function getVisitedViews() {
  const json = localStorage.getItem(CacheKey.VISITED_VIEWS)
  return JSON.parse(json ?? "[]") as TagView[]
}

export function setVisitedViews(views: TagView[]) {
  views.forEach((view) => {
    // 删除不必要的属性，防止 JSON.stringify 处理到循环引用
    delete view.matched
    delete view.redirectedFrom
  })
  localStorage.setItem(CacheKey.VISITED_VIEWS, JSON.stringify(views))
}

export function getCachedViews() {
  const json = localStorage.getItem(CacheKey.CACHED_VIEWS)
  return JSON.parse(json ?? "[]") as string[]
}

export function setCachedViews(views: string[]) {
  localStorage.setItem(CacheKey.CACHED_VIEWS, JSON.stringify(views))
}
// #endregion

// #region 监控图表排序偏好
/**
 * 所有看板的排序共用一个 key，内部按 scope（app-stat / instance-stat）分组，
 * 每组存成对象而非裸数组，方便以后追加「隐藏哪些图」之类的字段。
 */
interface MetricChartPreference {
  order?: string[]
}

type MetricChartPreferenceMap = Record<string, MetricChartPreference>

function readMetricChartPreferences(): MetricChartPreferenceMap {
  try {
    const json = localStorage.getItem(CacheKey.METRIC_CHART_ORDER)
    if (!json) return {}
    const parsed = JSON.parse(json)
    return parsed && typeof parsed === "object" ? (parsed as MetricChartPreferenceMap) : {}
  } catch {
    // 手工改坏或跨版本残留时按「没有偏好」处理，不影响看板本身
    return {}
  }
}

function writeMetricChartPreferences(preferences: MetricChartPreferenceMap) {
  localStorage.setItem(CacheKey.METRIC_CHART_ORDER, JSON.stringify(preferences))
}

export function getMetricChartOrder(scope: string): string[] | null {
  const order = readMetricChartPreferences()[scope]?.order
  return Array.isArray(order) && order.length ? order : null
}

export function setMetricChartOrder(scope: string, order: string[]) {
  const preferences = readMetricChartPreferences()
  preferences[scope] = { ...preferences[scope], order }
  writeMetricChartPreferences(preferences)
}

export function removeMetricChartOrder(scope: string) {
  const preferences = readMetricChartPreferences()
  delete preferences[scope]
  writeMetricChartPreferences(preferences)
}
// #endregion
