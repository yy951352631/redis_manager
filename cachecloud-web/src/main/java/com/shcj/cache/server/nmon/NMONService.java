package com.shcj.cache.server.nmon;

import com.shcj.cache.server.data.OS;
import com.shcj.cache.server.data.OSInfo;
import com.shcj.cache.ssh.SSHTemplate.DefaultLineProcessor;
import com.shcj.cache.ssh.SSHTemplate.Result;
import com.shcj.cache.ssh.SSHTemplate.SSHSession;
import com.shcj.cache.util.ConstUtils;
import com.shcj.cache.util.NMONFileFactory;
import com.shcj.cache.util.OSFactory;
...
public class NMONService {
    ...
    //nmon工作目录
    public static final String NMON_BASE_DIR= ConstUtils.REDIS_BASE_DIR + "/nmon";
    //获取系统版本位数命令
    public static final String OS_INFO_CMD = "/bin/uname -a; /bin/cat /etc/issue";
    //nmon路径
    public static final String NMON_DIR = ConstUtils.REDIS_BASE_DIR + "/bin/";
    //nmon文件名
    ...
    //nmon输出的结果文件
    public static final String NMON_LOG = NMON_BASE_DIR + "/nmon.log";
    //nmon输出的老结果文件
    public static final String NMON_OLD_LOG = NMON_BASE_DIR + "/nmon.old.log";
    //tcp输出的结果文件
    public static final String SOCK_LOG = NMON_BASE_DIR + "/sock.log";
    //ulimit输出的结果文件
    public static final String ULIMIT_LOG = NMON_BASE_DIR + "/ulimit.log";
    ...
    //创建nmon路径
    public static final String MK_NMON_DIR = "/bin/mkdir -p " + NMON_DIR;
    ...
    /**
     * nmon 二进制损坏（0 字节）时采集只会写入 sock/ulimit，不会产生监控曲线。
     */
    private void ensureNmonBinary(String ip, SSHSession session) {
        Result check = session.executeCommand(
                "[ -s \"" + NMON_FILE + "\" ] && stat -c%s \"" + NMON_FILE + "\" || echo 0");
        if (check.isSuccess() && check.getResult() != null) {
            try {
                long size = Long.parseLong(check.getResult().trim().split("\\s+")[0]);
                if (size >= 1024 && getNMONVersion(ip, session) != null) {
                    return;
                }
        ...
        logger.warn("{} nmon binary missing or invalid at {}, try redeploy", ip, NMON_FILE);
        initNmon(ip, session);
        trySystemNmonCopy(ip, session);
    }

    private void trySystemNmonCopy(String ip, SSHSession session) {
        String cmd = "if [ ! -s \"" + NMON_FILE + "\" ] || [ $(stat -c%s \"" + NMON_FILE
                + "\") -lt 1024 ]; then "
                + "if command -v nmon >/dev/null 2>&1; then cp -f $(command -v nmon) \"" + NMON_FILE
                + "\"; elif [ -x /usr/bin/nmon ]; then cp -f /usr/bin/nmon \"" + NMON_FILE + "\"; fi; "
                + "chmod +x \"" + NMON_FILE + "\"; fi";
        Result result = session.executeCommand(cmd);
        if (!result.isSuccess()) {
            logger.error("copy system nmon to {} failed: {}", ip, result.getResult(), result.getExcetion());
        }
    }
    ...
    private OSInfo initNmon(String ip, SSHSession session) {
        ...
        if (nmonFile != null && nmonFile.exists() && nmonFile.length() >= 1024) {
            sendNMONToServer(ip, session, nmonFile);
        } else {
            logger.warn("{} bundled nmon file unavailable, fallback to system nmon", ip);
            trySystemNmonCopy(ip, session);
        }
