package com.shcj.cache.task.entity;
...
public class TaskQueue {
    ...
    /**
     * 状态，详见com.shcj.cache.task.constant.TaskQueueEnum.TaskStatusEnum
     */
    private int status;
    ...
    public String getProgress() {
        double percent = getProgressValue();
        if (percent < 0) {
            return "";
        }
    ...
    public double getProgressValue() {
        ...
        return getProgressDoneSteps() * 100.0 / total;
    }

    public int getProgressDoneSteps() {
        if (CollectionUtils.isEmpty(taskStepFlowList)) {
            return 0;
        }
        int success = 0;
    ...
    public int getProgressTotalSteps() {
        return CollectionUtils.isEmpty(taskStepFlowList) ? 0 : taskStepFlowList.size();
    }

    public boolean isFinished() {
        return isSuccess() || isAbort() || status == TaskStatusEnum.TERMINATE.getStatus();
    }
