...
public interface TaskQueueDao {
    ...
    /**
     * 查询指定父任务类名下尚未结束（新建/准备执行/运行中）的任务。
     * 用于在子任务尚未落库时，仍能向前端展示"执行中"的诊断任务。
     *
     * @param className  父任务类名，见 DiagnosticTypeEnum#getParentTaskClassName
     * @param appId      可选，按应用过滤；传 null 表示不限
     * @return 按创建时间倒序
     */
    List<TaskQueue> getUnfinishedByClassName(@Param("className") String className, @Param("appId") Long appId);
