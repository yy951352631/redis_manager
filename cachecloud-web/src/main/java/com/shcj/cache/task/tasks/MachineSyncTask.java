...
public class MachineSyncTask extends BaseTask {
    ...
    public TaskFlowStatusEnum checkTargetMachineEnv() {
        ...
        try {
            ...
            if (checkResult.isSuccess() && checkResult.getResult().contains("cachecloud")) {
            ...
            if (checkRedisResult.isSuccess() && "0".equals(checkRedisResult.getResult())) {
    ...
    public TaskFlowStatusEnum startSyncData() {
        ...
        */
        String sync_command = " scp -i "+ConstUtils.DEFAULT_PUBLIC_KEY_PEM+" -oStrictHostKeyChecking=no -r " + baseConfDir.concat(containerIp) + " " + MachineCache.getUser(targetIp) + "@" + targetIp + ":" + baseConfDir +
                " && scp -i "+ConstUtils.DEFAULT_PUBLIC_KEY_PEM+"  -oStrictHostKeyChecking=no -r " + baseDataDir.concat(containerIp) + " " + MachineCache.getUser(targetIp) + "@" + targetIp + ":" + baseDataDir +
                " && scp -i "+ConstUtils.DEFAULT_PUBLIC_KEY_PEM+"  -oStrictHostKeyChecking=no -r " + baseLogDir.concat(containerIp) + " " + MachineCache.getUser(targetIp) + "@" + targetIp + ":" + baseLogDir;
