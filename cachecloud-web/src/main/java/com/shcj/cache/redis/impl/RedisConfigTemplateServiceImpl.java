...
import com.shcj.cache.dao.InstanceConfigDao;
...
import com.shcj.cache.redis.enums.RedisClusterConfigEnum;
import com.shcj.cache.redis.enums.RedisConfigEnum;
import com.shcj.cache.redis.enums.RedisSentinelConfigEnum;
...
public class RedisConfigTemplateServiceImpl implements RedisConfigTemplateService {
    ...
    public Boolean checkMachineRedisVersion(String host, String redisDir) {
        ...
        String cmds = "ls " + redisDir + "/src/redis-server | wc -l ";
        // 2.执行ssh命令
        try {
            ...
            throw new BizException("check redis-server machine ip:{}  execute cmds:{} error", host, cmds);
        }
    }
    ...
    public Boolean checkMachineRedisTool(String host, String redisDir) {
        ...
        try {
            ...
            throw new BizException("check checkMachineRedisTool machine ip:{}  execute cmds:{} error", host, cmds);
        }
    }
    ...
    public Boolean checkAndInstallRedisResource(String host, SystemResource redisResource) {
        ...
        boolean redisInstallFlag;

        // 检测是否安装需要redis版本
        String redisDir = ConstUtils.getRedisDir(redisResource.getName());

        Exception installException = new Exception();
        ...
        throw new BizException("installException={}, checkException={}", installException, checkException);
    }
    ...
    public Boolean checkAndInstallRedisTool(String host, SystemResource redisResource) {
        ...
        if (!checkMachineRedisTool(host, redisDir)) {
            ...
            for (int retry = 1; retry <= WAITING_RETRY_TIMES; retry++) {
                ...
                redisInstallFlag = false;
                logger.info("checkAndInstallRedisTool machine:{} install {} fail status:{} ,retry times :{} !", host, redisResource.getName(), redisInstallFlag, retry);
                try {
                    TimeUnit.SECONDS.sleep(WAITING_RESOURCE_SECOND);
                } catch (InterruptedException e) {
                    e.printStackTrace();
    ...
    public String installRedisOnMachine(String host, SystemResource systemResource) {
        ...
        if (repository == null) {
            throw new BizException("resourceService.getRepository() is empty");
        }
        // 2.资源信息拼接
        ...
        try {
            ...
            if (!result.isSuccess()) {
                throw new BizException("installRedisOnMachine failed host={}, execute_cmd={}", host, execute_cmd);
            }
            logger.info("execute_cmd :{} result:{}", execute_cmd, result);
        } catch (SSHException e) {
            throw new BizException("install redis machine ip:{}  execute cmd:{} error", host, download_cmd);
        }
        return SuccessEnum.SUCCESS.info();
    }
    ...
    public Map<String, Object> slaveUpdateConfig(long appId, Integer upgradeVersionId, String upgradeVersionName) {
        ...
        if (instanceList != null && instanceList.size() > 0) {
            for (InstanceInfo instance : instanceList) {
                if (instance.getStatus() == InstanceStatusEnum.GOOD_STATUS.getStatus() && "slave".equals(instance.getRoleDesc())) {
                    // slave节点 & instanceId
                    ...
                    instanceLogBuilder.append("<a target='_blank' href=/#/instance/detail/")
...
//        logger.info("result:{}", resultMap);
    ...
    @Override
    public void backupAofRdb(Long appId) {
        List<InstanceInfo> instanceInfoList = appService.getAppInstanceInfo(appId);
        String timeStr = DateUtil.formatYYYYMMddHHMMss(new Date());
        String backupAofCmd = "cp -rp " + ConstUtils.REDIS_BASE_DIR + "/data/appendonly-" + "%s" + ".aof " + ConstUtils.REDIS_BASE_DIR + "/data/appendonly-" + "%s" + ".aof_bak_" + timeStr;
        String backupRdbCmd = "cp -rp " + ConstUtils.REDIS_BASE_DIR + "/data/dump-" + "%s" + ".rdb " + ConstUtils.REDIS_BASE_DIR + "/data/dump-" + "%s" + ".rdb_bak_" + timeStr;

        for (InstanceInfo instanceInfo : instanceInfoList) {
            try {
                sshService.execute(instanceInfo.getIp(), String.format(backupAofCmd, String.valueOf(instanceInfo.getPort()), String.valueOf(instanceInfo.getPort())));
                sshService.execute(instanceInfo.getIp(), String.format(backupRdbCmd, String.valueOf(instanceInfo.getPort()), String.valueOf(instanceInfo.getPort())));
            } catch (SSHException ex) {
                logger.error("backupAofRdb fail ---> {}, {}", instanceInfo.getIp(), instanceInfo.getPort());
