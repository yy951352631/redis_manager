...
public class AppDeployCenterImpl implements AppDeployCenter {
    ...
    @Autowired
    private AppWeiXinService appWeiXinService;
    @Autowired
    ...
    @Override
    public boolean createApp(AppDesc appDesc, AppUser appUser, String memSize, String maxMemoryPolicy) {
        ...
        //内存保留策略
        appAudit.setParam3(maxMemoryPolicy);
        ...
        //鑫易通待审批通知
        appWeiXinService.sendApproveMsg(appDesc, appAudit);

        return true;
    }

    @Override
    public long createAppForAdminDeploy(AppDesc appDesc, AppUser appUser, String memSize, String maxMemoryPolicy) {
        appService.save(appDesc);
        String officers = appDesc.getOfficer();
        if (!StringUtils.isEmpty(officers)) {
            for (String officerId : officers.split(",")) {
    ...
    @Override
    public DataFormatCheckResult checkAppDeployDetail4Api(AppInfoApi appInfoApi, String appDeployText, com.shcj.cache.entity.RedisVersion redisVersion) {
