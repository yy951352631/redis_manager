...
import com.shcj.cache.redis.RedisClusterNode;
...
import com.shcj.cache.redis.enums.RedisConfigEnum;
import com.shcj.cache.redis.util.AuthUtil;
import com.shcj.cache.redis.util.JedisUtil;
...
import com.shcj.cache.util.ThreadUtil;
...
import com.shcj.cache.web.enums.UseTypeEnum;
...
public class RedisDeployCenterImpl implements RedisDeployCenter {
    ...
    private boolean clusterMeet(Jedis jedis, long appId, String host, int port) {
        ...
        boolean isMeet = response != null && "OK".equalsIgnoreCase(response);
        if (!isMeet) {
            ...
            if (index++ >= perSize || slot == 16383) {
                ...
                boolean isSlot = new IdempotentConfirmer() {
                    ...
                    public boolean execute() {
                        ...
                        boolean isSlot = response != null && "OK".equalsIgnoreCase(response);
                        if (!isSlot) {
                            return false;
                        }
                        return true;
        ...
        for (Map.Entry<Jedis, Jedis> entry : clusterMap.entrySet()) {
            ...
            boolean isReplicate = new IdempotentConfirmer() {
                ...
                public boolean execute() {
                    ...
                    boolean isReplicate = response != null && "OK".equalsIgnoreCase(response);
                    if (!isReplicate) {
                        try {
                            //等待广播节点
                            TimeUnit.SECONDS.sleep(2);
    ...
    private InstanceInfo saveInstance(long appId, String host, int port, int maxMemory, int type,
                                      String cmd) {
        return saveInstance(appId, host, port, maxMemory, type, cmd, 0);
    }

    private InstanceInfo saveInstance(long appId, String host, int port, int maxMemory, int type,
                                      String cmd, int parentInstanceId) {
        ...
        if (parentInstanceId > 0) {
            instanceInfo.setMasterInstanceId(parentInstanceId);
        }
        ...
        try {
            boolean isConfig = new IdempotentConfirmer() {
                ...
                public boolean execute() {
                    ...
                    boolean isConfig = result != null && "OK".equalsIgnoreCase(result);
                    if (!isConfig) {
                ...
                try {
                    setSlotStatus = new IdempotentConfirmer() {
                        ...
                        public boolean execute() {
                            ...
                            // result
                            boolean nodeSetStatus = setSlotsResult != null && "OK".equalsIgnoreCase(setSlotsResult);
...
//                redisCenter.checkAndLoadModule(appId, slaveHost, slavePort);
        ...
        if (TypeUtil.isRedisCluster(type)) {
            saveInstance(appId, slaveHost, slavePort, instanceInfo.getMem(),
                    ConstUtils.CACHE_TYPE_REDIS_CLUSTER, "", instanceId);
        } else {
            saveInstance(appId, slaveHost, slavePort, instanceInfo.getMem(),
                    ConstUtils.CACHE_REDIS_STANDALONE, "", instanceId);
        }
        return true;
    }
    ...
    private boolean hasSentinelInstance(List<InstanceInfo> instanceList) {
    ...
    public boolean sentinelFailover(final long appId) throws Exception {
        return runSentinelFailover(appId, null, null, null);
    }

    @Override
    public boolean sentinelSlaveFailover(final long appId, int slaveInstanceId, String failoverParam) throws Exception {
        Assert.isTrue(appId >= 0);
        Assert.isTrue(slaveInstanceId > 0);
        InstanceInfo slaveInfo = instanceDao.getInstanceInfoById(slaveInstanceId);
        if (slaveInfo == null) {
            logger.warn("appId={} slaveInstanceId={} not found", appId, slaveInstanceId);
            return false;
        }
        if (slaveInfo.getAppId() != appId) {
            logger.warn("appId={} slaveInstanceId={} belongs to appId={}", appId, slaveInstanceId, slaveInfo.getAppId());
            return false;
        }
        if (slaveInfo.getMasterInstanceId() <= 0) {
            logger.warn("appId={} slaveInstanceId={} is not a valid slave", appId, slaveInstanceId);
            return false;
        }
        if (TypeUtil.isRedisSentinel(slaveInfo.getType())) {
            logger.warn("instance {} is sentinel, skip slave failover", slaveInstanceId);
            return false;
        }
        if (slaveInfo.getStatus() != InstanceStatusEnum.GOOD_STATUS.getStatus()) {
            logger.warn("appId={} slaveInstanceId={} status is not online", appId, slaveInstanceId);
            return false;
        }
        if (StringUtils.isNotBlank(failoverParam)
                && !"force".equalsIgnoreCase(failoverParam)
                && !"takeover".equalsIgnoreCase(failoverParam)) {
            logger.error("appId={} invalid sentinel failoverParam={}", appId, failoverParam);
            return false;
        }
        BooleanEnum isMaster = redisCenter.isMaster(appId, slaveInfo.getIp(), slaveInfo.getPort());
        if (BooleanEnum.TRUE == isMaster) {
            logger.warn("appId={} slaveInstanceId={} is already master", appId, slaveInstanceId);
            return false;
        }
        if (BooleanEnum.OTHER == isMaster) {
            logger.warn("appId={} slaveInstanceId={} role unknown, skip failover", appId, slaveInstanceId);
            return false;
        }
        logger.warn("sentinelSlaveFailover appId={}, slave={}:{} param={}",
                appId, slaveInfo.getIp(), slaveInfo.getPort(), failoverParam);
        return runSentinelFailover(appId, failoverParam, slaveInfo.getIp(), slaveInfo.getPort());
    }

    /**
     * 向 Sentinel 发起 Failover。指定从节点时依赖 Redis Sentinel 6.2+：
     * SENTINEL FAILOVER &lt;master&gt; [FORCE|TAKEOVER] &lt;ip&gt; &lt;port&gt;
     */
    private boolean runSentinelFailover(final long appId, final String failoverParam,
                                        final String targetSlaveHost, final Integer targetSlavePort) throws Exception {
        Assert.isTrue(appId >= 0);
        AppDesc appDesc = appDao.getAppDescById(appId);
        Assert.isTrue(appDesc != null);
        ...
        int type = appDesc.getType();
        if (!TypeUtil.isRedisSentinel(type) && !hasSentinelInstance(instanceList)) {
        ...
        for (InstanceInfo instanceInfo : instanceList) {
            ...
            if (!redisCenter.isSentinelRun(appId, host, port)) {
            ...
            boolean isSentinelFailOver = new IdempotentConfirmer() {
                @Override
                public boolean execute() {
                    Jedis jedis = openSentinelJedis(appDesc, host, port);
                    try {
                        return invokeSentinelFailover(jedis, masterName, failoverParam, targetSlaveHost, targetSlavePort);
                    } finally {
                        jedis.close();
                    }
                }
            ...
            if (!isSentinelFailOver) {
                logger.warn("{}:{} sentinel failover error, masterName={}, target={}:{} param={}",
                        host, port, masterName, targetSlaveHost, targetSlavePort, failoverParam);
                return false;
            }
            logger.warn("SentinelFailOver done via {}:{}, masterName={}, target={}:{} param={}",
                    host, port, masterName, targetSlaveHost, targetSlavePort, failoverParam);
            return true;
        }
        return false;
    ...
    private Jedis openSentinelJedis(AppDesc appDesc, String host, int port) {
        if (appDesc.hasSentinelPwdFlag()) {
            return redisCenter.getJedis(host, port, appDesc.getAppPassword());
        }
        return redisCenter.getJedis(host, port);
    }

    private boolean invokeSentinelFailover(Jedis jedis, String masterName, String failoverParam,
                                           String targetSlaveHost, Integer targetSlavePort) {
        try {
            String response;
            if (StringUtils.isBlank(targetSlaveHost) || targetSlavePort == null) {
                response = jedis.sentinelFailover(masterName);
            } else {
                String portStr = String.valueOf(targetSlavePort);
                if (StringUtils.isBlank(failoverParam)) {
                    response = toStatusReply(jedis.sendCommand(Protocol.Command.SENTINEL,
                            "FAILOVER", masterName, targetSlaveHost, portStr));
                } else if ("force".equalsIgnoreCase(failoverParam)) {
                    response = toStatusReply(jedis.sendCommand(Protocol.Command.SENTINEL,
                            "FAILOVER", masterName, "FORCE", targetSlaveHost, portStr));
                } else {
                    response = toStatusReply(jedis.sendCommand(Protocol.Command.SENTINEL,
                            "FAILOVER", masterName, "TAKEOVER", targetSlaveHost, portStr));
                }
            }
            return response != null && "OK".equalsIgnoreCase(response);
        } catch (JedisDataException e) {
            if (StringUtils.isNotBlank(targetSlaveHost)) {
                logger.error("sentinel slave failover failed, masterName={}, target={}:{} param={}, need Redis Sentinel 6.2+: {}",
                        masterName, targetSlaveHost, targetSlavePort, failoverParam, e.getMessage());
            } else {
                logger.error("sentinel failover failed, masterName={}: {}", masterName, e.getMessage());
            }
            return false;
        }
    }

    private String toStatusReply(Object response) {
        if (response == null) {
            return null;
        }
    ...
    public boolean sentinelReset(long app_id) throws Exception {
        ...
        for (InstanceInfo instanceInfo : instanceList) {
            ...
            if (TypeUtil.isRedisSentinel(instanceType)) {
                ...
                boolean isRun = redisCenter.isSentinelRun(app_id, host, port);
                if (!isRun) {
                    logger.warn("{}:{} is not run", host, port);
                    continue;
                }
                boolean isSentinelReset = new IdempotentConfirmer() {
                    ...
                    public boolean execute() {
                        Jedis jedis;
                        if (appDesc.hasSentinelPwdFlag()) {
                            jedis = redisCenter.getJedis(host, port, appDesc.getAppPassword());
                        } else {
            ...
            boolean isForget = new IdempotentConfirmer() {
                ...
                public boolean execute() {
                    ...
                    try {
                        ...
                        boolean success = response != null && "OK".equalsIgnoreCase(response);
...
//        }
    ...
    public boolean fixPassword(Long appId, String newPkey) {
        ...
        boolean isSuccess = batchFixPassword(instanceInfos, oldPasswordMD5, newPasswordMD5, appDesc.hasSentinelPwdFlag());
        if (isSuccess) {
    ...
    public boolean fixPassword(Long appId, String password, Boolean customPwdFlag, boolean initRedisFlag) {
        ...
        if (!initRedisFlag) {
            for (InstanceInfo instanceInfo : instanceInfos) {
                if (instanceInfo.isStop() || instanceInfo.isError()) {
                    throw new BizException("存在停止或心跳终止的实例={}", instanceInfo.getHostPort());
                }
            }
        }

        boolean isSuccess = batchFixPassword(instanceInfos, oldPassword, newPassword, appDesc.hasSentinelPwdFlag());
        if (!customPwdFlag) {
            if (isSuccess) {
                appDesc.setPkey(newPkey);
    ...
    public boolean checkAuths(Long appId) {
        ...
        for (InstanceInfo instanceInfo : instanceInfos) {
            ...
            try {
                ...
                if (TypeUtil.isRedisSentinel(type)) {
                    Jedis jedis = redisCenter.getSentinelJedis(appId, host, port);
                    nodeList.add(jedis);
                }
            } catch (Exception e) {
    ...
    private boolean batchFixPassword(List<InstanceInfo> instanceInfos, String oldPasswordMD5, String passwordMD5, boolean hasSentinelPwd) {
        ...
        for (InstanceInfo instanceInfo : instanceInfos) {
            ...
            try {
                ...
                if (TypeUtil.isRedisCluster(type) || TypeUtil.isRedisStandalone(type)) {
                    ...
                    Jedis jedis;
                    if (hasSentinelPwd) {
                        jedis = redisCenter.getJedis(host, port, oldPasswordMD5);
                    } else {
                        jedis = redisCenter.getJedis(host, port);
                    }
        ...
        try {
            ...
            if (StringUtils.isNotBlank(masterName)) {
                for (Jedis jedis : sentinelList) {
                    boolean isFix = fixSentinelPassword(jedis, masterName, passwordMD5, hasSentinelPwd);
                    if (isFix) {
                        //加入待回滚列表
                        ...
                        rollbackNodes(rollbackNodeList, oldPasswordMD5);
                        rollbackSentinels(rollbackSentinelList, masterName, oldPasswordMD5, hasSentinelPwd);
                        return false;
                    }
                }
            }
    ...
    private boolean fixSentinelPassword(Jedis jedis, String masterName, String passwordMD5, boolean hasSentinelPwd) {
        ...
        try {
            ...
            if ("OK".equals(setResult)) {
                ...
                if (hasSentinelPwd) {
                    updateSentinelPassword(jedis, passwordMD5);
                }
    ...
    private void updateSentinelPassword(Jedis jedis, String passwordMD5) {
        //todo zoushunqing
        // 删除requirepass 和 user default on nopass
        // 增加requirepass
        // 停止sentinel
        // 启动sentinel
        // 重置jedis
        ...
        logger.info("host:{} port:{} res shutdown:{}", host, port, jedis.shutdown());
        ThreadUtil.sleepSec(3);

        String configFile = String.format(RedisProtocol.getConfig(port, false));
        String delRequirePass = "sed -i '/requirepass/d' " + ConstUtils.REDIS_BASE_DIR + "/conf/" + configFile;
        String delNoPass = "sed -i '/user default on nopass/d' " + ConstUtils.REDIS_BASE_DIR + "/conf/" + configFile;
        String delUserPass = "sed -i '/user default on /d' " + ConstUtils.REDIS_BASE_DIR + "/conf/" + configFile;
        String addRequirePass = "sed -i '7i requirepass " + passwordMD5 + "' " + ConstUtils.REDIS_BASE_DIR + "/conf/" + configFile;
        logger.info("host:{} port:{} res: delRequirePass:{} {}", host, port, machineCenter.executeShell(host, delRequirePass), delRequirePass);
        logger.info("host:{} port:{} res: delUserPass:{} {}", host, port, machineCenter.executeShell(host, delUserPass), delUserPass);
        logger.info("host:{} port:{} res: delNoPass:{} {}", host, port, machineCenter.executeShell(host, delNoPass), delNoPass);
        if (StringUtils.isNotBlank(passwordMD5)) {
            logger.info("host:{} port:{} res addRequirePass:{} {}", host, port, machineCenter.executeShell(host, addRequirePass), addRequirePass);
        }

        InstanceInfo instanceInfo = instanceDao.getInstByIpAndPort(host, port);
        logger.info("host:{} port:{} res startInstance:{}", host, port, instanceDeployCenter.startInstance(instanceInfo.getAppId(), instanceInfo.getId()));
    ...
    private void rollbackSentinels(List<Jedis> rollbackSentinelList, String masterName, String oldPasswordMD5, boolean hasSentinelPwd) {
        // rollback-sentinels
        for (Jedis rollbackJedis : rollbackSentinelList) {
            boolean rollback = fixSentinelPassword(rollbackJedis, masterName, oldPasswordMD5, hasSentinelPwd);
            logger.warn("sentinel-rollback: node={} rollback={}",
                    JedisUtil.getHostPort(rollbackJedis), rollback);
            updateSentinelPassword(rollbackJedis, oldPasswordMD5);
        }
    }
    ...
    private boolean checkAuthNodes(List<Jedis> nodes, String passwordMD5) {
        for (Jedis jedis : nodes) {
            try {
                ...
                logger.info("checkAuthNodes--->{}", auth);
                if (!"OK".equals(auth)) {
                    return false;
                }
            } catch (JedisDataException e) {
                ...
                throw new BizException("node-auth-failed: node={} password={} error={}", JedisUtil.getHostPort(jedis), passwordMD5, e.getMessage());
            }
        }
        return true;
    }
    ...
    public boolean slaveOf(final long appId, final String masterHost, final int masterPort, final String slaveHost,
                           final int slavePort) {
        try (Jedis slave = redisCenter.getJedis(appId, slaveHost, slavePort, Protocol.DEFAULT_TIMEOUT * 3, Protocol.DEFAULT_TIMEOUT * 3)) {
            ...
            throw new BizException("Jedis slaveOf command failed, masterHost={}, masterPort={}, slaveHost={}, slavePort={}", masterHost, masterPort, slaveHost, slavePort);
        }
        return true;
    }
