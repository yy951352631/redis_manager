...
public class RedisServerKeyTtlAnalysisTask extends BaseTask {
    ...
    public TaskFlowStatusEnum keyTtlAnalysis() {
        ...
        try {
            ...
            ttlTimeCountMap.asMap().forEach((enumVal, ttlCount) -> {
                String ttlDistri = enumVal.getValue();
                ttlCounts.put(ttlDistri, ttlCount);
                logger.info(marker, "{} {} {}:{} ttl distri {} {}", keyTtlResultKey, appId, host, port, ttlDistri, ttlCount);
            });
            flushAssistZsetCounts(keyTtlResultKey, ttlCounts, "TTL");
            requireLocalStatsWhenDbNonEmpty(dbSize, !ttlTimeCountMap.isEmpty(), host, port, "TTL");
