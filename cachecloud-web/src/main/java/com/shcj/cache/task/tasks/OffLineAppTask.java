...
public class OffLineAppTask extends BaseTask {
    ...
    public TaskFlowStatusEnum init() {
        ...
        if (userInfo == null) {
            ...
            if (!AppUserTypeEnum.ADMIN_USER.value().equals(userInfo.getType())) {
                logger.error("task {} appId {} user {} who hope to offline hasn't admin privilege", taskId, appId, userInfo.getName());
                taskFlowStatusEnum = TaskFlowStatusEnum.ABORT;
            }
        }
    ...
    public TaskFlowStatusEnum executeOffLineApp() {
        ...
        markAllInstancesOfflineByApp(appId);
        return TaskFlowStatusEnum.SUCCESS;
    }

    /** 集群下线：将该集群下所有节点同步标记为已下线 */
    private void markAllInstancesOfflineByApp(long appId) {
        List<InstanceInfo> instanceInfos = instanceDao.getInstListByAppId(appId);
        if (instanceInfos == null || instanceInfos.isEmpty()) {
            return;
        }
        for (InstanceInfo instanceInfo : instanceInfos) {
            if (instanceInfo.getStatus() == InstanceStatusEnum.OFFLINE_STATUS.getStatus()) {
                continue;
            }
            instanceInfo.setStatus(InstanceStatusEnum.OFFLINE_STATUS.getStatus());
            instanceDao.update(instanceInfo);
            logger.info("cluster offline cascade appId={} instance {}:{} -> offline",
                    appId, instanceInfo.getIp(), instanceInfo.getPort());
        }
    }

    private boolean instanceOffline(InstanceInfo instanceInfo, int type) {  //todo zoushunqing 加 一个配置文件备份
        final String ip = instanceInfo.getIp();
        final int port = instanceInfo.getPort();
        if (redisCenter.requiresPlatformOnlyOffline(instanceInfo)) {
            logger.info("task {} appId {} {}:{} platform-only offline (no host ssh)", taskId, appId, ip, port);
            instanceInfo.setStatus(InstanceStatusEnum.OFFLINE_STATUS.getStatus());
            instanceDao.update(instanceInfo);
            return true;
        }
