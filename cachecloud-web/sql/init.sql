...
(
    ...
    `sentinel_password` varchar(255)      DEFAULT NULL COMMENT 'Sentinel 自身的密码，与数据节点密码分开',
...
-- 运行期自建表（风险评估 / 离线分析 / 压测）
--
-- 这些表原先只由后端启动时的 DatabaseSchemaInitializer 建出来，init.sql 里没有，
-- 于是「读一遍 init.sql」看不到完整表结构，离线部署也得先把服务拉起来才有表。
-- 这里补齐，与 DatabaseSchemaInitializer 和 sql/risk-assess-schema.sql 保持一致。
--
-- 全部使用 CREATE TABLE IF NOT EXISTS：启动时那段逻辑仍会执行，重复建表不报错，
-- 两条路径谁先跑都可以。
-- =============================================================================
...
(
    ...
    `peak_concurrency`        int(11)       NOT NULL DEFAULT '0' COMMENT '峰值所在并发档',
    ...
    `peak_target_cpu_percent` double        NOT NULL DEFAULT '0' COMMENT '全程承压节点峰值CPU(单核%)',
