...
import com.shcj.cache.web.service.ToolService;
...
public class ToolServiceImpl implements ToolService {
    ...
    private void sentinelExam(final long appId, List<Map> examResult) {
        ...
        for (InstanceInfo inst : instances) {
            if (inst.getStatus() == 1 && "sentinel".equals(inst.getRoleDesc())) {
            ...
            } else if (inst.getStatus() == 1 && "master".equals(inst.getRoleDesc())) {

            } else if (inst.getStatus() == 1 && "slave".equals(inst.getRoleDesc())) {
    ...
    private void clusterExam(final long appId, List<Map> examResult) {
        ...
        for (InstanceInfo inst : instances) {
            if (inst.getStatus() == 1) {//实例正常启用
                ...
                } else if ("slave".equals(inst.getRoleDesc())) {
                    slaveNum++;
                    String masterHost = inst.getMasterHost();
