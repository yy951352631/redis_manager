...
/** 侧栏菜单：跳过 hidden 的布局容器，直接展示其子路由 */
const noHiddenRoutes = computed(() => {
  const result: RouteRecordRaw[] = []
  permissionStore.routes.forEach((item) => {
    if (item.meta?.hidden) {
      item.children?.forEach((child) => {
        if (!child.meta?.hidden && child.meta?.title) result.push(child)
      })
      return
    }
    result.push(item)
...
function menuItemBasePath(item: RouteRecordRaw) {
...
const sidebarMenuActiveBgColor = computed(() => !isTop.value ? "var(--v3-sidebar-menu-active-bg-color)" : "transparent")
    ...
    <el-scrollbar wrap-class="scrollbar-wrapper">
      ...
      >
        <Item
          ...
          :item="noHiddenRoute"
          :base-path="menuItemBasePath(noHiddenRoute)"
        />
      </el-menu>
    </el-scrollbar>
  </div>
...
:deep(.el-menu--horizontal .el-menu-item) {
  ...
  &.is-active {
    background-color: v-bind(sidebarMenuActiveBgColor);
  }
}
