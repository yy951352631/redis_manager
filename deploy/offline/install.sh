...
: "${CC_REDIS_PASSWORD:=}"
: "${CC_REDIS_SENTINEL_MASTER:=}"
: "${CC_REDIS_SENTINEL_NODES:=}"
: "${CC_REDIS_SENTINEL_PASSWORD:=}"
: "${CC_JAVA_XMS:=512m}"
: "${CC_JAVA_XMX:=1536m}"
: "${CC_PROFILE:=online}"
...
render "$HERE/conf/setenv.sh" "$TOMCAT_HOME/bin/setenv.sh" \
  ...
  "REDIS_SENTINEL_MASTER=$CC_REDIS_SENTINEL_MASTER" \
  "REDIS_SENTINEL_NODES=$CC_REDIS_SENTINEL_NODES" \
  "REDIS_SENTINEL_PASSWORD=$CC_REDIS_SENTINEL_PASSWORD" \
  "CC_HOME=$CC_HOME"
