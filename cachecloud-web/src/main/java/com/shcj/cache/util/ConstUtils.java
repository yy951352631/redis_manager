...
public class ConstUtils {
    public static final String NOTICE_TITLE = "【Redis统一管理平台】状态通知";
     ...
     * 服务端版本
     */
    public static final String CACHECLOUD_VERSION = "1.0.0";
    /**
     * 逗号
     */
     ...
     * 内部错误
     */
    public static final String INNER_ERROR = "redis_cloud_inner_error";
    /**
     * 登录跳转参数
     */
    ...
    public static final String REDIS_BASE_DIR = "/home/redis";
    public static final String REDIS_DEFAULT_BIN = REDIS_BASE_DIR + "/bin";
    public final static String REDIS_DEFAULT_DIR = REDIS_BASE_DIR + "/redis";
    /**
     * redis安装基准目录
     */
    public static final String REDIS_INSTALL_BASE_DIR = REDIS_BASE_DIR;
    public static final String REDIS_COMPILE_BASE_DIR = REDIS_BASE_DIR + "/compile/";
    /**
     * pika 安装基准目录,挂载ssd
     */
    public static final String PIKA_INSTALL_BASE_DIR = REDIS_BASE_DIR + "/pika";
    /**
     * nutcracker当期版本
     */
     ...
     */
    public static final int DEFAULT_SSH_AUTH_TYPE = SshAuthTypeEnum.PASSWORD.getValue();
    /**
     * public key pem
     */
    public static final String DEFAULT_PUBLIC_KEY_PEM = REDIS_BASE_DIR + "/ssh/id_rsa";
    public static final String DEFAULT_PUBLIC_USERNAME = "redis";
     ...
     * module info
     */
    public static final String MODULE_BASE_PATH = REDIS_BASE_DIR + "/module/";
     ...
     * 系统间token
     */
    public static CopyOnWriteArraySet<String> NJCB_TOKEN_SET = new CopyOnWriteArraySet<>();
    ...
    /**
     * 各类型占用内存（字节，来自 value 扫描）
     */
    public static String getRedisServerTypeMemoryKey(long appId, long auditId) {
        return String.format("cc:key:typeMemory:%s:%s", appId, auditId);
    }

    /**
     * Top N 大 key（zset，score=字节数，member=ip\\u0001port\\u0001type\\u0001keyName）
     */
    public static String getRedisServerTopKeyKey(long appId, long auditId) {
        return String.format("cc:key:top:%s:%s", appId, auditId);
    }

    /** 键值分析风险/告警信息（LIST，任务写入、结果页读取） */
    public static String getKeyAnalysisRiskKey(long appId, long auditId) {
        return String.format("cc:key:risk:%s:%s", appId, auditId);
    }

    private static final String TOP_KEY_MEMBER_SEP = "\u0001";

    public static String encodeTopKeyMember(String ip, int port, String type, String keyName) {
        return ip + TOP_KEY_MEMBER_SEP + port + TOP_KEY_MEMBER_SEP + StringUtils.defaultString(type) + TOP_KEY_MEMBER_SEP + keyName;
    }

    public static String[] decodeTopKeyMember(String member) {
        if (StringUtils.isBlank(member)) {
            return null;
        }
        String[] parts = member.split(TOP_KEY_MEMBER_SEP, 4);
        if (parts.length < 4) {
            return null;
        }
        return parts;
