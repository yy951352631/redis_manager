...
public class InstanceApiService {
    ...
    private Double instanceMetricValue(Map<String, Object> row, String name) {
        ...
        if (InstanceStatsCenterImpl.READ_COMMAND_STAT.equals(name)
            ...
            // 复用集群图表那份实现：这里原先抄了一份一模一样的循环，改口径时漏改，
            // 同一份数据在集群页和节点页给出的答案对不上。
            Long total = InstanceStatsCenterImpl.sumCommandStats(
                    row, InstanceStatsCenterImpl.WRITE_COMMAND_STAT.equals(name));
