...
import com.shcj.cache.task.constant.ScanCleanConstants;
...
public class AppScanCleanKeyTask extends BaseTask {
    ...
    public TaskFlowStatusEnum init() {
        ...
        if (operateType == null) {
            throw new BizException("task {} operateType is illegal", taskId);
        }
        boolean nodesAndJudge = getNodesAndJudge(operateType);
        if (!nodesAndJudge) {
            throw new BizException("task {} pointed nodes is illegal", taskId);
        }
    ...
    public TaskFlowStatusEnum createAppScanCleanKeyTask() {
        ...
        for (RedisServerNode redisServerNode : redisServerNodes) {
            ...
            try {
                ...
                logger.info(marker, "appId {} {}:{} dbsize:{} redis scanCleanKeyTask create successfully childTaskId={}", appId, host, port, dbSize, childTaskId);
            } catch (Exception e) {
                throw new BizException("appId {} {}:{}  redis scanCleanKeyTask create fail", appId, host, port, e);
            }
        }
        return TaskFlowStatusEnum.SUCCESS;
    }
    ...
    public TaskFlowStatusEnum waitAppScanCleanKeyTaskFinish() {
        for (RedisServerNode redisServerNode : redisServerNodes) {
            ...
            if (taskFlowStatusEnum.equals(TaskFlowStatusEnum.ABORT)) {
                throw new BizException("appId {} {}:{} instanceScanKeyTask execute fail, childTaskId={}", appId, host, port, childTaskId);
            } else {
