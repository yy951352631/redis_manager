...
function pointValue(p: ChartPoint) { return p.yDouble ?? p.ydouble ?? p.y ?? 0 }
function lineData(points: ChartPoint[]) { return points.filter(p => p.x != null).map(p => [p.x!, pointValue(p)] as [number, number]) }
...
function memBarClass(ratio: number) { return ratio >= 80 ? "is-danger" : "is-success" }
...
async function fetchData() {
  ...
  try {
    ...
    await renderCharts()
  } finally { loading.value = false; chartsLoading.value = false }
}
...
watch(() => props.instanceId, () => { dateRange.value = defaultRange(); void fetchData() }, { immediate: true })
...
<template>
  <div v-loading="loading" class="app-tab-embed app-instance-stat-page app-stat-page">
    ...
    <p class="range-hint">时间范围最长 7 天</p>

    <div v-if="stat" class="panels-row app-instance-overview-panels">
      <div class="panel-col app-instance-overview-left">
        <div class="page-header"><h4>节点信息-所属集群 <el-link type="primary" :underline="false" @click="emit('openApp', stat.appId)">[{{ stat.appName }}]</el-link></h4></div>
        <table class="app-instance-summary-table app-instance-summary-table--stacked"><tbody>
          <tr><td>内存使用率</td><td><div class="instance-mem-progress"><div class="instance-mem-progress__bar" :class="memBarClass(stat.memUsePercent)" :style="{ width: `${Math.min(stat.memUsePercent, 100)}%` }"><label>{{ stat.memUsedGb.toFixed(2) }}G Used/{{ stat.memTotalGb.toFixed(2) }}G Total</label></div></div></td></tr>
        ...
        <table class="app-instance-summary-table app-instance-summary-table--stacked app-instance-runtime-table"><tbody>
        ...
        <table class="app-stat-info-table"><tbody>
          <tr><td>命令</td><td>峰值QPM</td><td colspan="2">峰值产生时间</td></tr>
          <tr v-for="row in stat.top5Climax" :key="row.commandName"><td>{{ row.commandName }}</td><td>{{ row.commandCount.toLocaleString() }}</td><td colspan="2">{{ row.createTime || "-" }}</td></tr>
          <tr v-if="!stat.top5Climax?.length"><td colspan="4" class="instance-command-empty">暂无数据</td></tr>
        </tbody></table>
      </div>
      <div class="panel-col">
...
<style scoped>
.app-instance-overview-panels { align-items: start; }
.app-instance-overview-left { min-width: 0; }
.app-instance-summary-table--stacked { table-layout: fixed; }
.app-instance-summary-table--stacked td:first-child { width: 108px !important; }
.app-instance-runtime-table { margin: 12px 0 20px; }
.app-instance-runtime-table td:nth-child(3) { width: 108px !important; }
.instance-command-empty { text-align: center; color: var(--rp-text-muted); }
.os-arch-hint { margin-left: 4px; color: var(--rp-text-muted); vertical-align: -2px; cursor: help; }
</style>
