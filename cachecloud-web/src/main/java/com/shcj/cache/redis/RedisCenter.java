...
public interface RedisCenter {
    ...
    public boolean isSentinelRun(final long appId, String ip, int port);
    ...
    /**
     * 主机是否具备 SSH 管控能力（纳管机器且配置了 SSH 账号）
     */
    boolean isHostSshCapable(String ip);

    /**
     * 无 SSH 管控时是否仅做平台侧下线（不远程 SHUTDOWN 进程）
     */
    boolean requiresPlatformOnlyOffline(InstanceInfo instanceInfo);
    ...
    List<Map<String, Object>> formatClientList(List<String> clientList, boolean includeConnectionDetails);
    ...
    Jedis getSentinelJedis(long appId, String host, int port);
    ...
    boolean shutdownSentinel(long appId, String host, int port);
