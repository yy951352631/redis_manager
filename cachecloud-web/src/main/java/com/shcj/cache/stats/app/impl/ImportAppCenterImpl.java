...
import com.shcj.cache.stats.app.ImportAppCenter;
...
public class ImportAppCenterImpl implements ImportAppCenter {
    ...
    public ImportAppResult check(int type, String appInstanceInfo, String password) {
        ...
        for (String appInstance : appInstanceDetails) {
            ...
            if (exists != null) {
                String addr = ip + ":" + port;
                if (exists.getStatus() == InstanceStatusEnum.OFFLINE_STATUS.getStatus()
                        || exists.getStatus() == InstanceStatusEnum.FORGET_STATUS.getStatus()) {
                    return ImportAppResult.fail("节点 " + addr + " 已下线，无法再次添加");
                }
                return ImportAppResult.fail("节点 " + addr + " 已在平台中纳管，无法重复添加");
            }
