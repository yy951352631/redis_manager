...
public class InstanceScanCleanKeyTask extends BaseTask {
    ...
    private static final Pattern LESS_MATCH_PATTERN = Pattern.compile("@Less\\{[0-9]+\\}Less@");
    private static final Pattern MORE_MATCH_PATTERN = Pattern.compile("@More\\{[0-9]+\\}More@");
    ...
    private void compilePatternAndSet() {
        Matcher lessMatcher = LESS_MATCH_PATTERN.matcher(pattern);
        ...
        if (foundFlag) {
            ...
            Matcher moreMatcher = MORE_MATCH_PATTERN.matcher(pattern);
