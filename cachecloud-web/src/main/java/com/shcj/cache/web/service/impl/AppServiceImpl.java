...
public class AppServiceImpl implements AppService {
    ...
    public AppDesc getByAppId(Long appId) {
        ...
        return resolveApp(appId);
    }

    private long normalizeAppId(long idOrClusterNo) {
        Long resolved = resolveAppId(idOrClusterNo);
        return resolved != null ? resolved : idOrClusterNo;
    }

    @Override
    public AppDesc resolveApp(long idOrClusterNo) {
        if (AppClusterNoSupport.isClusterNo(idOrClusterNo)) {
            AppDesc byClusterNo = appDao.getByClusterNo((int) idOrClusterNo);
            if (byClusterNo != null) {
                return byClusterNo;
            }
        }
        return appDao.getAppDescById(idOrClusterNo);
    }

    @Override
    public Long resolveAppId(long idOrClusterNo) {
        AppDesc appDesc = resolveApp(idOrClusterNo);
        return appDesc != null ? appDesc.getAppId() : null;
    }

    @Override
    public int save(AppDesc appDesc) {
        if (appDesc.getClusterNo() == null || appDesc.getClusterNo() <= 0) {
            appDesc.setClusterNo(appDao.allocateNextClusterNo());
        }
    ...
    @Override
    public boolean waitSentinelTopologySync(long appId, int promotedSlaveInstanceId, long timeoutMs, long intervalMs) {
        appId = normalizeAppId(appId);
        long deadline = System.currentTimeMillis() + timeoutMs;
        boolean synced = false;
        while (System.currentTimeMillis() < deadline) {
            List<InstanceInfo> list = getAppInstanceInfo(appId);
            if (promotedSlaveInstanceId > 0) {
                for (InstanceInfo info : list) {
                    if (info.getId() == promotedSlaveInstanceId
                            && info.getStatus() == InstanceStatusEnum.GOOD_STATUS.getStatus()
                            && "master".equals(info.getRoleDesc())) {
                        synced = true;
                        break;
                    }
                }
                ...
                for (InstanceInfo info : list) {
                    if (info.getStatus() != InstanceStatusEnum.GOOD_STATUS.getStatus()
                            || TypeUtil.isRedisSentinel(info.getType())) {
                        continue;
                    }
                    if ("master".equals(info.getRoleDesc())) {
                        masterCount++;
                    }
                }
                synced = masterCount == 1;
            }
            if (synced) {
                logger.info("sentinel topology synced for appId={}, promotedSlaveInstanceId={}",
                        appId, promotedSlaveInstanceId);
                int updated = syncAppTopology(appId);
                logger.info("sentinel topology parent_id synced for appId={}, updated={}", appId, updated);
                return true;
            }
            try {
        ...
        syncAppTopology(appId);
        logger.warn("sentinel topology sync timeout for appId={}, promotedSlaveInstanceId={}",
                appId, promotedSlaveInstanceId);
        return false;
    }

    @Override
    public int syncAppTopology(long appId) {
        appId = normalizeAppId(appId);
        return applyParentIdSync(appId, false);
    }

    @Override
    public int countAppTopologyDrift(long appId) {
        appId = normalizeAppId(appId);
        return applyParentIdSync(appId, true);
    }

    @Override
    public int syncAllOnlineAppTopology() {
        ...
        for (AppDesc app : apps) {
            if (!needsTopologyParentSync(app.getType())) {
                continue;
            }
            int updated = syncAppTopology(app.getAppId());
            if (updated > 0) {
                logger.warn("scheduled topology sync appId={} name={} instancesUpdated={}",
                        app.getAppId(), app.getName(), updated);
                totalUpdated += updated;
            }
        }
        logger.info("syncAllOnlineAppTopology finished, totalInstancesUpdated={}", totalUpdated);
        return totalUpdated;
    }

    private boolean needsTopologyParentSync(int appType) {
        return appType == ConstUtils.CACHE_TYPE_REDIS_CLUSTER
                || appType == ConstUtils.CACHE_REDIS_SENTINEL
                || appType == ConstUtils.CACHE_REDIS_STANDALONE;
    }

    private int applyParentIdSync(long appId, boolean dryRun) {
        AppDesc appDesc = appDao.getAppDescById(appId);
        if (appDesc == null) {
            return 0;
        }
        ...
        Map<Integer, Integer> dbParentById = new HashMap<Integer, Integer>();
        for (InstanceInfo info : resultList) {
            dbParentById.put(info.getId(), info.getMasterInstanceId());
        }
        getInstancelistInfo(appId, appDesc.getAppPassword(), resultList);
        int changed = 0;
        for (InstanceInfo info : resultList) {
            if (!shouldSyncParentId(info)) {
                continue;
            }
            Integer dbParent = dbParentById.get(info.getId());
            if (dbParent == null) {
                continue;
            }
            int expectedParent = info.getMasterInstanceId();
            ...
            try {
                instanceDao.updateParentId(info.getId(), expectedParent);
                changed++;
                logger.info("sync parent_id appId={} instanceId={} {} -> {}", appId, info.getId(), dbParent,
                        expectedParent);
            } catch (Exception e) {
                logger.error("sync parent_id failed appId={} instanceId={} targetParent={}: {}", appId, info.getId(),
                        expectedParent, e.getMessage(), e);
            }
        }
        return changed;
    }

    private boolean shouldSyncParentId(InstanceInfo info) {
        return info.getStatus() == InstanceStatusEnum.GOOD_STATUS.getStatus()
                && TypeUtil.isRedisType(info.getType())
                && !TypeUtil.isRedisSentinel(info.getType());
    }
    ...
    public List<InstanceInfo> getInstancelistInfo(Long appId, String password, List<InstanceInfo> resultList) {
        ...
        for (InstanceInfo instanceInfo : resultList) {
            ...
            if (!TypeUtil.isRedisType(type) || TypeUtil.isRedisSentinel(type)) {
                continue;
            }
            ...
            try {
                if (type == ConstUtils.CACHE_REDIS_STANDALONE || type == ConstUtils.CACHE_TYPE_REDIS_CLUSTER) {
                    ...
                    modules = modules.stream().sorted(Comparator.comparing(Module::getName))
        ...
        for (InstanceInfo instanceInfo : resultList) {
            ...
            HostAndPort hap = redisCenter.getMaster(instanceInfo.getIp(), instanceInfo.getPort(), password);
            attachReplicationMaster(instanceInfo, hap, resultList);
        }
        return resultList;
    }

    @Override
    public int resolveReplicationMasterInstanceId(String masterHost, int masterPort, int slaveInstanceId,
                                                  List<InstanceInfo> instances) {
        Integer masterId = lookupReplicationMasterInstanceId(masterHost, masterPort, slaveInstanceId, instances);
        return masterId != null ? masterId : 0;
    }

    private void attachReplicationMaster(InstanceInfo slave, HostAndPort hap, List<InstanceInfo> resultList) {
        if (hap == null) {
            return;
        }
        slave.setMasterHost(hap.getHost());
        slave.setMasterPort(hap.getPort());
        Integer masterId = lookupReplicationMasterInstanceId(hap.getHost(), hap.getPort(), slave.getId(), resultList);
        if (masterId != null) {
            slave.setMasterInstanceId(masterId);
        }
    }

    private Integer lookupReplicationMasterInstanceId(String masterHost, int masterPort, int slaveInstanceId,
                                                      List<InstanceInfo> instances) {
        if (masterPort <= 0 || CollectionUtils.isEmpty(instances)) {
            return null;
        }
        for (InstanceInfo candidate : instances) {
            if (!isTopologyLinkCandidate(candidate) || candidate.getId() == slaveInstanceId) {
                continue;
            }
            if (java.util.Objects.equals(candidate.getIp(), masterHost) && candidate.getPort() == masterPort) {
                return candidate.getId();
            }
        }
        if (isLoopbackHost(masterHost)) {
            return findUniqueMasterByPort(masterPort, slaveInstanceId, instances);
        }
        return findUniqueMasterByPort(masterPort, slaveInstanceId, instances);
    }

    private Integer findUniqueMasterByPort(int masterPort, int slaveInstanceId, List<InstanceInfo> instances) {
        Integer match = null;
        for (InstanceInfo candidate : instances) {
            if (!isTopologyLinkCandidate(candidate) || candidate.getId() == slaveInstanceId) {
                continue;
            }
            if (candidate.getPort() != masterPort) {
                continue;
            }
            if (!"master".equals(candidate.getRoleDesc())) {
                continue;
            }
            if (match != null) {
                return null;
            }
            match = candidate.getId();
        }
        return match;
    }

    private boolean isTopologyLinkCandidate(InstanceInfo info) {
        return info.getStatus() == InstanceStatusEnum.GOOD_STATUS.getStatus()
                && TypeUtil.isRedisType(info.getType())
                && !TypeUtil.isRedisSentinel(info.getType());
    }
    ...
    public AppAudit saveInstanceChangeConfig(AppDesc appDesc, AppUser appUser, Long instanceId,
        ...
        appAudit.setInfo(
                "appId=" + appId + "下的" + hostPort + "节点申请修改配置项:" + instanceConfigKey + ", 配置值: " + instanceConfigValue
    ...
    public AppAudit saveRegisterUserApply(AppUser appUser, AppAuditType registerUserApply) {
        ...
        info.append(appUser.getChName() + "申请成为Redis统一管理平台用户, 手机:" + appUser.getMobile() + ",邮箱:" + appUser.getEmail()
    ...
    @Override
    public SuccessEnum changeAppAlertConfig(long appId, int isAccessMonitor, AppUser appUser) {
        appId = normalizeAppId(appId);
        if (appId <= 0) {
            return SuccessEnum.FAIL;
        }
        ...
        try {
            // 报警阈值已统一收口到「报警配置」，这里只保留集群级报警通知开关
    ...
    public void getMachineCandiList(MachineMemStatInfo memStatInfo, Double reqSize, Integer reqCpu, Integer isSalve,
        ...
        int mem = memStatInfo.getMem(); //单位G
        long applyMen = memStatInfo.getApplyMem();//单位bit
        long availMen = mem * 1024L - applyMen / 1024 / 1024;////单位M
        int availCpu = memStatInfo.getCpu() - memStatInfo.getInstanceNum();
     ...
     * @update 必须有slave by zoushunqing
     */
    @Override
    public List<DeployInfo> generateInstanceInfo(List<String> redisMasterMachineList, List<String> redisSlaveMachineList, int type, int appType, int masterNum, int maxMemory, List<DeployInfo> deployInfoList) {
        //错开，防止与主节点同机器
        ...
        if (!CollectionUtils.isEmpty(redisMasterMachineList)) {
            for (int index = 0; index < masterNum; index++) {
                DeployInfo deployInfo = null;
                if (DeployInfo.isRedisNode(type)) {
                    deployInfo = DeployInfo.getRedisInfo(appType, redisMasterMachineList.get(index), maxMemory, redisSlaveMachineList.get(index));
                } else {
                    deployInfo = DeployInfo.getPikaInfo(appType, redisMasterMachineList.get(index), maxMemory, redisSlaveMachineList.get(index));
                }
                if (deployInfo != null) {
                    deployInfoList.add(deployInfo);
                }
    ...
    private boolean appClientStatGatherFilter(Map<String, Object> appClientGatherStat) {
        ...
        try {
            ...
            return !appService.getByAppId(appId).isTestOk();
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
            return false;
        }
    ...
    public AppAudit saveAppKeyAnalysis(AppDesc appDesc, AppUser appUser, String appAnalysisReason, String nodeInfo) {
        ...
        appAudit.setModifyTime(new Date());
        appAudit.setInfo(StringUtils.hasText(appAnalysisReason) ? appAnalysisReason : "键值分析");
        appAudit.setStatus(AppCheckEnum.APP_WATING_CHECK.value());
    ...
    @Override
    public List<InstanceInfo> listDefaultKeyAnalysisInstances(long appId) {
        final long internalAppId = normalizeAppId(appId);
        List<InstanceInfo> instances = getAppInstanceInfo(internalAppId).stream()
                .filter(InstanceInfo::isRedisData)
                .collect(Collectors.toList());
        if (instances.isEmpty()) {
            return instances;
        }
        List<InstanceInfo> slaves = instances.stream()
                .filter(inst -> redisCenter.isMaster(internalAppId, inst.getIp(), inst.getPort()) == BooleanEnum.FALSE)
                .collect(Collectors.toList());
        return slaves.isEmpty() ? instances : slaves;
    }
    ...
    @Override
    public void updateAppVersion(Long appId, Integer versionId) {
        long internalAppId = normalizeAppId(appId);
        AppDesc appDescById = appDao.getAppDescById(internalAppId);
        if (appDescById == null) {
            throw new BizException("没有appId={}的应用", internalAppId);
        }
        appDescById.setVersionId(versionId);
        appDao.update(appDescById);
    }
}
