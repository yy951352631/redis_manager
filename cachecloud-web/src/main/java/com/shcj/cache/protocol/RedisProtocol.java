...
import com.shcj.cache.redis.cli.RedisCliProperties;
...
public class RedisProtocol {
    ...
    /**
     * 2019-05-14 k8s容器启动路径  REDIS_BASE/conf/${host}/redis-${port}.conf
     *
     * @param host
     * @param port
     * @param isCluster
    ...
    public static String getExecuteCommandShell(String host, int port, String password, String command) {
        ...
        shell.append(String.format(resolveRemoteCliPath() + " -h %s -p %s ", host, port));
    ...
    private static String resolveRemoteCliPath() {
        try {
            if (SpringContextUtil.getApplicationContext() != null) {
                RedisCliProperties properties = (RedisCliProperties) SpringContextUtil.getBeanByClass(RedisCliProperties.class);
                return properties.getRemoteCliPath();
            }
        } catch (Exception ignored) {
        }
        return ConstUtils.REDIS_DEFAULT_BIN + "/redis-cli";
    }
