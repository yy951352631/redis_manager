...
public class DateUtil {
    ...
    /**
     * 解析页面时间区间参数，支持 yyyy-MM-dd HH:mm:ss、yyyy-MM-dd HH:mm、yyyy-MM-dd
     */
    public static Date parseRangeDateTime(String dateStr) throws ParseException {
        if (StringUtils.isBlank(dateStr)) {
            return null;
        }
        String text = dateStr.trim().replace('+', ' ');
        if (text.length() >= 19) {
            return parse(text.substring(0, 19), "yyyy-MM-dd HH:mm:ss");
        }
        if (text.length() >= 16) {
            return parse(text.substring(0, 16), "yyyy-MM-dd HH:mm");
        }
        return parseYYYY_MM_dd(text);
    }

    public static String formatRangeDateTime(Date date) {
    ...
    public static TimeBetween fillWithDateFormat(String searchDate) throws ParseException {
        ...
        if (StringUtils.isBlank(searchDate)) {
            ...
            startDate = com.shcj.cache.web.util.DateUtil.parse(searchDate, dateFormat);
        }
        endDate = DateUtils.addDays(startDate, 1);
        // 查询后台需要
        long startTime = NumberUtils.toLong(com.shcj.cache.web.util.DateUtil.formatDate(startDate, COLLECT_TIME_FORMAT));
        long endTime = NumberUtils.toLong(com.shcj.cache.web.util.DateUtil.formatDate(endDate, COLLECT_TIME_FORMAT));
