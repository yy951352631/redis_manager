...
public class NMONFileFactory {
	...
	/**
	 * 资源仓库不可达时，回退使用内置 nmon（优先 centos7，其次任意可用包）。
	 */
	public static File getBundledNmonFile() {
		String[] preferredKeys = {
				"linux_x86_64_centos7",
				"linux_x86_64_linux_old",
				"linux_x86_64_linux"
		};
		for (String key : preferredKeys) {
			File file = nmonFileMap.get(key);
			if (isValidNmonFile(file)) {
				return file;
			}
		}
		for (File file : nmonFileMap.values()) {
			if (isValidNmonFile(file)) {
				return file;
			}
		}
	...
	private static boolean isValidNmonFile(File file) {
